...
class Iterator:
    ...
    def __init__(self, task):
        ...
        self.context_output = os.path.join(self.CWD,task,"2023_GT_vis_context_labels_v1")        
        self.deeplabOutputDir= os.path.join(self.CWD,task,"2023_GT_labeled_images")   #2023_labeled_images_test  
        self.ctxConsensusDir = os.path.join(self.CWD,task,"ctx_consensus")   
        self.ctxCOMPConsensusDir = os.path.join(self.CWD,task,"vis_context_labels_v5_pred")   
    ...
    def GenerateContext(self,SAVE=False):
        ...
        for Trial in FilenamesInTask:
            ...
            ctxCOMPFName = os.path.join(self.ctxCOMPConsensusDir, Trial+".txt")  
            ...
            for root, dirs, files in os.walk(TrialRoot):
                for file in files:
                    ...
                    # All _GT_points contains the contours that are applicable for this Task and Trial combo                    
                    All_GT_points = DeeplabVIAPoints[trialFname] #["class"]["frame"] = list of points
                    ...
                    threadRoot = root.replace("images","2023_GT_thread_masks")
                    ...
                    try:
                        LgrasperPoints = All_GT_points["2023_grasper_L"][str(frameNumber)]
                    except Exception as e:
                        print(e,"in LgrasperPoints")
                        LgrasperPoints = {}

                    try:
                        RgrasperPoints = All_GT_points["2023_grasper_R"][str(frameNumber)]
                    except Exception as e:
                    ...
                    if "Needle" in self.task or "Suturing" in self.task:
                        try:                        
                            NeedlePoints = All_GT_points["2023_needle"][str(frameNumber)]
                        except Exception as e:
                    ...
                    CtxICOMP = utils.ContextInterface2(ctxCOMPFName)
                    ...
                    if("Suturing" in self.task):              
                        ...
                        contextLines.append(ctxPredLine)
                        #print(Trial,frameNumber,ctxPredLine)
                        #self.DrawSingleImageContextS(pred, gt,needleShape,needleShape_gt,GrasperJawPoints,imageFName,outputFName,CtxI,CtxICOMP,ctxPredLine,frameNumber,L_Gripping,R_Gripping,LG_inter_T, RG_inter_T,messages)
                    count += 1
        ...
        if GT:
            ...
            try:
                ...
                for i in range(len(ringShapes)):
                    ...
                    if not isinstance(R_GROUP,list) and not isinstance(needleShape,list) and len(needleShape.geoms) != 0:
                ...
                if(not isinstance(needleShape,list) and not isinstance(ringShapes,list)):
                    RingDistances_N = [ min([needleShape.distance(shape) for shape in R_GROUP.geoms if not isinstance(needleShape,list) ])  for R_GROUP in ringShapes if not isinstance(R_GROUP,list)] if ringShapes != [] else []
                    RingInter_N =  [ max([needleShape.intersection(shape).area for shape in R_GROUP.geoms if not isinstance(needleShape,list)])  for R_GROUP in ringShapes  if not isinstance(R_GROUP,list)] if ringShapes != [] else []
                else: 
                    RingDistances_N = [300]
    ...
    def DrawSingleImageContextS(self,pred, gt,needleShape,needleShape_gt,GrasperJawPoints,imageFName,outputFName,CtxI,CtxICOMP,ctxPredLine,frameNumber,L_Gripping,R_Gripping,LG_inter_T, RG_inter_T,messages,GT=False):
        [LG_dl,RG_dl,T_dl] = pred
        [LG_Group_gt,RG_Group_gt,T_Group_gt] = gt
        image = cv.imread(imageFName)
        ...
        contextMessage = ["Ground Truth:"," ".join(ctxGT),"Pre-Store Context:"," ".join(ctxPred),"DeepLab ICRA23:"," ".join(ctxCOMP)]
        offset = 1
        fs = 7
        ctxOffset = 350
        caxis = plt.gca()
    ...
    def DrawSingleImageContextKT(self, pred, gt,GrasperJawPoints,imageFName,outputFName,CtxI,ctxPredLine,frameNumber,L_Gripping,R_Gripping,LG_inter_T, RG_inter_T,GT=False):
        ...
        '''
        LG_GT_s = gpd.GeoSeries(LG_dl)
        LG_GT_s.plot()
        RG_GT_s = gpd.GeoSeries(RG_dl)
        RG_GT_s.plot()
        T_GT_s = gpd.GeoSeries(T_dl)
        T_GT_s.plot()
